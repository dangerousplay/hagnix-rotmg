...
namespace server.account
{
    internal class register : RequestHandler
    {
        ...
        {
            if (Query["ignore"] == null ||
                !string.IsNullOrWhiteSpace(Query["entrytag"])
                || string.IsNullOrWhiteSpace(Query["isAgeVerified"])
                || !Query["newGUID"].Contains("@")
                || Environment.GetEnvironmentVariable("DISABLE_REGISTER") != null)
            {
                ...
                else
                {
                    ...
                    if (acc != null)
                    {
                        if (Program.Settings.GetValue<bool>("verifyEmail"))
                        {
                            ...
                            message.Body = "<center>Please verify your email via this <a href=\"" +
                                           Program.Settings.GetValue<string>("serverDomain", "localhost") +
                                           "/account/validateEmail?authToken=" + acc.AuthToken +
        ...
        public bool IsValidEmail(string strIn)
        {
            ...
            {
                var s = strIn.Remove(0, strIn.IndexOf('@') + 1);
                var emailBody = s.Remove(s.IndexOf('.'));
                switch (emailBody)
                {
                    case "mail":
                    case "yandex":
