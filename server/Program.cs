...
namespace server
{
    internal class Program
    {
        ...
        internal static ISimpleSettings Settings { get; set; }
...
//        internal static ILog Logger { get; } = LogManager.GetLogger("Server");
        internal static NLog.Logger Logger { get; } = NLog.LogManager.GetCurrentClassLogger();
...
//            XmlConfigurator.ConfigureAndWatch(new FileInfo("log4net_server.config"));
        ...
        private static bool RunPreCheck(int port) =>
            IPGlobalProperties.GetIPGlobalProperties().GetActiveTcpConnections()
                .All(_ => _.LocalEndPoint.Port != port) && IPGlobalProperties.GetIPGlobalProperties()
                .GetActiveTcpListeners().All(_ => _.Port != port);
        ...
        private static void ProcessRequest(HttpListenerContext context)
        {
            try
            {
                ...
                if (context.Request.Url.LocalPath.IndexOf(".") == -1)
                    ...
                    s = "server" + context.Request.Url.LocalPath.Remove(context.Request.Url.LocalPath.IndexOf("."))
                            .Replace("/", ".");
                if ((t = Type.GetType(s)) == null)
                {
                    using (var wtr = new StreamWriter(context.Response.OutputStream))
                    {
                        ...
                        if (info.Attributes == FileAttributes.Directory)
                        {
                            ...
                            file = (from f in files
                                let inf = new FileInfo(f)
                                where inf.Name.Replace(inf.Extension, string.Empty)
                        ...
                        if (File.Exists(file))
                        {
                            if (file.StartsWith("game/Testing.html"))
                                file = Settings.GetValue<bool>("testingOnline", "false")
                                    ? "game/Testing.html"
                                    : "game/TestingIsOffline.html";
                ...
                else
                {
                    ...
                    if (!(handler is RequestHandler))
                    {
                        if (handler == null)
                            ...
                            using (var wtr = new StreamWriter(context.Response.OutputStream))
                                wtr.Write("<Error>Class \"{0}\" is not of the type RequestHandler.</Error>",
                                    t.FullName);
                    }
                    else
        ...
        public static void SendFile(string path, HttpListenerContext context)
        {
            ...
            if (context.Response.ContentType == "text/html" || context.Response.ContentType == "text/javascript" ||
        ...
        public static void SendEmail(System.Net.Mail.MailMessage message, bool enableSsl)
        {
            try
            {
                ...
                {
                    client.Connect(Settings.GetValue<string>("smtpHost"), Settings.GetValue<int>("smtpPort"),
                        enableSsl);
                    client.Authenticate(new NetworkCredential(Settings.GetValue<string>("serverEmail"),
