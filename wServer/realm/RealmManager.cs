...
//        private static readonly ILog log = LogManager.GetLogger(typeof(RealmManager));
        ...
        public ConcurrentDictionary<string, long> AuthorizedList { get; private set; }
        ...
        public RealmManager(int maxClients, int tps)
        {
            ...
            AuthorizedList = new ConcurrentDictionary<string, long>();
            Random = new Random();
        }
        ...
        public Player FindPlayerByEmail(string email)
        {
            return (from i in Worlds
                    where i.Key != 0
                    from e in i.Value.Players
                    where String.Equals(e.Value.Client.Account.Email, email, StringComparison.CurrentCultureIgnoreCase)
                    select e.Value).FirstOrDefault();
        }
        ...
        private void CheckList()
        {
            ...
            timer.Elapsed += (E,R) => {
                foreach(var v in AuthorizedList)
                {
                    if(v.Value <= DateTimeOffset.Now.ToUnixTimeMilliseconds())
                    {
                        AuthorizedList.TryRemove(v.Key, out _);

                        FindPlayerByEmail(v.Key).Client.Disconnect();
                    }
                }
            };
