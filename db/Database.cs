    ...
    public partial class Database : IDisposable
    {
        ...
        public Database(string host, string database, string user, string password)
        {
            ...
            while (true)
                ...
                {
                    Log.Warn($"Can't connect to MySQL: {e}");
                    Thread.Sleep(1000);
                }
        ...
        public Database()
        {
            ...
            while (true)
                ...
                {
                    _con.Open();
                    Log.Info($"Connected on MySQL: {_host}");
                    break;
                }
                catch (Exception e)
                {
...
//            if (TimeZoneInfo.Local.Id != TimeZoneInfo.FindSystemTimeZoneById("Pacific Standard Time").Id)
        ...
        public Account Register(string uuid, string password, string objectId, bool isGuest, XmlData data)
        {
            ...
            cmd = CreateQuery();
            cmd.CommandText =
                "INSERT INTO `accounts`(`uuid`, `password`, `object_id`, `name`, `rank`, `namechosen`, `verified`, `guild`, `guildRank`, `guildFame`, `vaultCount`, `maxCharSlot`, `regTime`, `guest`, `banned`, `locked`, `ignored`, `gifts`, `isAgeVerified`, `authToken`) VALUES(@uuid, SHA1(@password), @object_id, @randomName, @rank, 0, 0, 0, 0, 0, 1, 2, @regTime, @guest, 0, @empty, @empty, @empty, 1, @authToken);";
            cmd.Parameters.AddWithValue("@uuid", uuid);
        ...
        public Account GetAccountByUUID(string uuid, XmlData data)
        {
            MySqlCommand cmd = CreateQuery();
            cmd.CommandText =
